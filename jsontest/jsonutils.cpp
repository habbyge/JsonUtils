...
CJsonUtils::CJsonUtils()
    : m_bRoot(true), m_pDoc(new Document()), m_pValue(new Value(kObjectType)) {}

CJsonUtils::CJsonUtils(Document* pDoc, Value* pValue)
    : m_bRoot(false), m_pDoc(pDoc), m_pValue(pValue) {}

CJsonUtils::CJsonUtils(const CJsonUtils& other)
    : m_bRoot(false), m_pDoc(other.m_pDoc), m_pValue(other.m_pValue) {}
...
bool CJsonUtils::Parse(const char* strJsonText) {
  if (false == m_bRoot) return false;

  m_pDoc->Parse(strJsonText);
  if (kParseErrorNone != m_pDoc->GetParseError()) return false;
...
bool CJsonUtils::IsString() const { return m_pValue->IsString(); }

bool CJsonUtils::IsNumber() const { return m_pValue->IsNumber(); }

bool CJsonUtils::IsStringNumber() const {
  if (false == IsString()) return false;
...
bool CJsonUtils::IsInt32() const { return m_pValue->IsInt(); }

bool CJsonUtils::IsInt64() const { return m_pValue->IsInt64(); }

bool CJsonUtils::IsUint32() const { return m_pValue->IsUint(); }

bool CJsonUtils::IsUint64() const { return m_pValue->IsUint64(); }

bool CJsonUtils::IsDouble() const { return m_pValue->IsDouble(); }
...
bool CJsonUtils::Remove(int32_t iArrayIndex) const {
  if (false == m_pValue->IsArray()) return false;

  if (m_pValue->Size() < iArrayIndex) return false;

  int index = 0;
  for (rapidjson::Value::ValueIterator iter = m_pValue->Begin();
       iter != m_pValue->End();) {
...
int32_t CJsonUtils::ArrayCount() const {
  if (false == m_pValue->IsArray()) return -1;

  return m_pValue->Size();
}

CJsonUtils CJsonUtils::operator[](int32_t iArrayIndex) {
  if (false == m_pValue->IsArray()) m_pValue->SetArray();
...
const CJsonUtils CJsonUtils::operator[](int32_t iArrayIndex) const {
  if (false == m_pValue->IsArray()) return CJsonUtils(m_pDoc, &gNullValue);
...
CJsonUtils& CJsonUtils::operator=(const std::string& strValue) {
  m_pValue->SetString(strValue.c_str(), strValue.length(),
                      m_pDoc->GetAllocator());
  return *this;
}
...
CJsonUtils::operator bool() const { return ConvertToNumber<bool>(atoi, 0); }

template <typename _TYPE_, typename _CONVERT_FUNC_, typename _DEF_VAL>
_TYPE_ CJsonUtils::ConvertToNumber(_CONVERT_FUNC_ pFunc,
                                   _DEF_VAL tDefValue) const {
  if (true == m_pValue->IsString()) {
...
CJsonUtilsW::CJsonUtilsW()
    : m_bRoot(true),
      m_pDoc(new DocumentW()),
      m_pValue(new ValueW(kObjectType)) {}

CJsonUtilsW::CJsonUtilsW(DocumentW* pDoc, ValueW* pValue)
    : m_bRoot(false), m_pDoc(pDoc), m_pValue(pValue) {}

CJsonUtilsW::CJsonUtilsW(const CJsonUtilsW& other)
    : m_bRoot(false), m_pDoc(other.m_pDoc), m_pValue(other.m_pValue) {}
...
bool CJsonUtilsW::IsString() const { return m_pValue->IsString(); }

bool CJsonUtilsW::IsNumber() const { return m_pValue->IsNumber(); }
...
bool CJsonUtilsW::IsInt32() const { return m_pValue->IsInt(); }

bool CJsonUtilsW::IsInt64() const { return m_pValue->IsInt64(); }

bool CJsonUtilsW::IsUint32() const { return m_pValue->IsUint(); }

bool CJsonUtilsW::IsUint64() const { return m_pValue->IsUint64(); }

bool CJsonUtilsW::IsDouble() const { return m_pValue->IsDouble(); }
...
bool CJsonUtilsW::Remove(int32_t iArrayIndex) const {
  ...
  int index = 0;
  for (ValueW::ValueIterator iter = m_pValue->Begin();
       iter != m_pValue->End();) {
    if (index == iArrayIndex) {
      m_pValue->Erase(iter);
      break;
...
const CJsonUtilsW CJsonUtilsW::operator[](int32_t iArrayIndex) const {
  if (false == m_pValue->IsArray()) return CJsonUtilsW(m_pDoc, &gNullValueW);
...
CJsonUtilsW::operator bool() const { return ConvertToNumber<bool>(_wtoi, 0); }

template <typename _TYPE_, typename _CONVERT_FUNC_, typename _DEF_VAL>
_TYPE_ CJsonUtilsW::ConvertToNumber(_CONVERT_FUNC_ pFunc,
                                    _DEF_VAL tDefValue) const {
  if (true == m_pValue->IsString()) {
    return pFunc(m_pValue->GetString());
  } else if (true == m_pValue->IsInt()) {
